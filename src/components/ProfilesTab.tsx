...
const ProfilesTab: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-6">
      <h2 className="text-2xl font-semibold mb-6 text-gray-800">Perfiles</h2>
      <div className="mb-6">
        <Input
          type="file"
          accept=".csv"
        ...
        <label htmlFor="csv-upload">
          <Button variant="outline" className="mb-4 w-full hover:bg-blue-50 hover:border-blue-200 transition-colors" asChild>
            <span>
      ...
      <div className="mb-6 space-y-3">
        <Input
          ...
          className="w-full border-gray-200 focus:border-blue-300 rounded-lg"
        />
        <Input
          type="number"
          placeholder="Edad"
        ...
        <div className="space-y-2 p-4 bg-gray-50 rounded-lg">
          {roles.map((role) => (
            <div key={role} className="flex items-center space-x-3">
              ...
              />
              <label htmlFor={role} className="text-sm font-medium text-gray-700 select-none">
                {role}
              </label>
            </div>
          ))}
        </div>
        <Button onClick={handleAddProfile} className="w-full bg-blue-600 hover:bg-blue-700">
          <Plus className="mr-2 h-4 w-4" /> Agregar Perfil
        </Button>
      </div>
      <div className="space-y-3">
        {profiles.map((profile) => (
          <div key={profile.id} className="bg-gray-50 p-4 rounded-lg border border-gray-100 flex justify-between items-center hover:border-blue-200 transition-colors">
            <div>
              <h3 className="text-lg font-semibold text-gray-800">{profile.name} <span className="text-gray-500">({profile.age} años)</span></h3>
              <p className="text-sm text-gray-600">Roles: {profile.roles.join(', ')}</p>
            </div>
            <div className="space-x-2">
              <Button variant="outline" size="sm" onClick={() => handleEditProfile(profile)} className="hover:bg-blue-50 hover:border-blue-200">
                <Edit className="h-4 w-4" />
              </Button>
              <Button variant="outline" size="sm" onClick={() => handleDeleteProfile(profile.id)} className="hover:bg-red-50 hover:border-red-200">
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            <Input
              ...
              value={newName}
              onChange={(e) => setNewName(e.target.value)}
              className="w-full border-gray-200 focus:border-blue-300"
            />
            <Input
              type="number"
              placeholder="Edad"
              ...
              {roles.map((role) => (
                <div key={role} className="flex items-center space-x-3">
                  <Checkbox
                    ...
                    className="border-gray-300 data-[state=checked]:bg-blue-600"
                  />
                  <label htmlFor={`edit-${role}`} className="text-sm font-medium text-gray-700 select-none">
                    {role}
                  </label>
                </div>
              ))}
